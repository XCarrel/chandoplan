    ...
    <div class="container m-1">
        ...
        @foreach($domains as $domain)
            ...
            @foreach($domain->activities as $activity)
                <div class="row m-1">
                    <h5 class="m-1 p-1 {{ $activity->domain->slug }}">{{ $activity->title }}</h5>{{ $activity->location }}, responsable: {{ $activity->responsible->name }}, {{ $activity->minparticipants }}-{{ $activity->maxparticipants }} p., {{ \App\Helpers\Helpers::localeDayOfWeek($activity->slot->date) }} {{ \Carbon\Carbon::parse($activity->slot->timeslot->from)->format('H:i') }}
